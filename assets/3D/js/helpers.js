...
var RESUME_JSON;
$.getJSON("assets/Global/data/resume.json", function(data) {
  RESUME_JSON = data
})
...
function buildSkyBox(scene) {
    ...
    skyboxMaterial.reflectionTexture = new BABYLON.CubeTexture("assets/3D/skybox/TropicalSunnyDay/skybox", scene);
...
function fixGlass(game, shadowGens) {
    ...
    for (var i = 0; i < game.scene.meshes.length; i++) {
        ...
        if (currMesh.name == "TV") {
            ...
            // currMat.diffuseTexture = new BABYLON.VideoTexture("video", ["assets/3D/video/screen.mp4"], scene, true);
            ...
            // currMesh.material.diffuseTexture = new BABYLON.VideoTexture("video", ["assets/3D/video/Team Carbon Frag Video.mp4"], scene, true);
        }
